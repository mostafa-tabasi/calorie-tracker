...
class ValidateNumber {
    fun isAgeValid(age: String): ValidationResult<Int> {
        val ageNumber = age.toIntOrNull() ?: run {
            return ValidationResult.Fail(UiText.StringResource(R.string.error_age_cant_be_empty))
        }
        if (ageNumber == 0)
            return ValidationResult.Fail(UiText.StringResource(R.string.error_age_cant_be_zero))
    ...
    fun isHeightValid(height: String): ValidationResult<Int> {
        val heightNumber = height.toIntOrNull() ?: run {
            return ValidationResult.Fail(UiText.StringResource(R.string.error_height_cant_be_empty))
        }
        if (heightNumber == 0)
            return ValidationResult.Fail(UiText.StringResource(R.string.error_height_cant_be_zero))
    ...
    fun isWeightValid(weight: String): ValidationResult<Float> {
        val weightNumber = weight.toFloatOrNull() ?: run {
            return ValidationResult.Fail(UiText.StringResource(R.string.error_weight_cant_be_empty))
        }

        if (weightNumber == 0f)
            return ValidationResult.Fail(UiText.StringResource(R.string.error_weight_cant_be_zero))
